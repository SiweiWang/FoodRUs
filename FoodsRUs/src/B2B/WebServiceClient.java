...
public class WebServiceClient {
	...
	public static void order(Collection<ItemBean> items) throws SOAPException, MalformedURLException {
		...
		for (ItemBean item: items)
		{
			...
			SOAPElement element = body.addChildElement("quote").addChildElement("itemNumber");
			...
			org.w3c.dom.Node tnode = toronto.getSOAPPart().getEnvelope().getBody()
					.getElementsByTagName("quoteReturn").item(0);
			org.w3c.dom.Node vnode = vancouver.getSOAPPart().getEnvelope().getBody()
					.getElementsByTagName("quoteReturn").item(0);
			org.w3c.dom.Node hnode = halifax.getSOAPPart().getEnvelope().getBody()
					.getElementsByTagName("quoteReturn").item(0);
			...
			System.out.println("Toronto " + tquote + " " + item.getItemNumber());
			System.out.println("Vancouver "+vquote + " " + item.getItemNumber());
			System.out.println("Halifax " + hquote +" " + item.getItemNumber());
		...
		for (ItemBean item: items)
		{
			...
			SOAPElement element = body.addChildElement("order").addChildElement("itemNumber");
			
			element.addTextNode(item.getItemNumber());
			element.addTextNode(String.valueOf(item.getQuantity()));
			...
			System.out.println(item.getItemNumber()+ " supplier " + item.getSupID() + " oder response ");
			if (node!= null) System.out.print(node.getTextContent());
			sc.close();
		}
	}
